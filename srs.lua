local SRS_LOADED = false
repeat wait() until game and game:IsLoaded()
if game:GetService("CoreGui"):FindFirstChild("SRS") then SRS_LOADED = nil return end
...
local AllFrame
local executed_succ,executed_reason = pcall(function()
	    			...
	    			for i2,v2 in pairs(found_tbls) do
	    				if type(v2[1]) == "table" then
	    					...
	    					for i3,v3 in pairs(v2[1]) do
	    						...
	    						if i3 == to_search_val or v3 == to_search_val or (tostring(i3) == tostring(to_search_val) and type(i3) ~= "number") or tostring(v3) == tostring(to_search_val) then
		    						local newfunc = RemoteLog:Clone()
						            newfunc.TextColor3 = Color3.new(0,1,1)
						            newfunc.BorderColor3 = Color3.new(0,1,1)
